...
export const sentFinalEstimation = async (req, res) => {
  try {
    ...
    const clientTime = new Date(Number(req.headers["client-time"])).getHours();
    console.log(clientTime);
    let dayoftime = "";
    const customerName = req.body.customerName;
    if (clientTime < 12) {
      dayoftime = "Morning";
    } else if (clientTime > 12 && clientTime < 16) {
      dayoftime = "Afternoon";
    } else {
      dayoftime = "Evening";
    }

    const pdfBase64 = req.body.pdf.split(",")[1];
    console.log(req.body.pdf.split(",")[0]);
    const fileUrl = await base64ToFile(
      pdfBase64,
      req.body.customerLeadId[0],
      "pdf",
      req.body.estimationNumber
    );
    ...
    <body>
      <div class="email-container">
        <table style="color: #000; font-size: 20px; margin-top: 80px !important">
          <tr>
            <td style="padding: 10px 14px">
              <h2>This is your Project Estimate</h2>
            </td>
          </tr>
          <tr>
            <td style="padding: 10px 14px">
              Good ${dayoftime},<b>${customerName}</b>,
            </td>
          </tr>
          <tr>
            <td style="padding: 10px 14px">
              Here you will find a PDF document and a link to see our estimate.
              You will be able to select and unselect the options.
            </td>
          </tr>
        ...
        <table style="color: #000; font-size: 20px">
          <tr>
            <td style="text-align: center; padding-bottom: 5px">
              <ul style="display: inline-block; padding: 0">
                <li style="display: inline-block; padding: 10px 15px">
                  <a href="#"
                    ><img src="http://digimonk.net:1630/facebook.png"
                  /></a>
                </li>
                <li style="display: inline-block; padding: 10px 15px">
                  <a href="#"
                    ><img src="http://digimonk.net:1630/twitter.png"
                  /></a>
                </li>
                <li style="display: inline-block; padding: 10px 15px">
                  <a href="#"
                    ><img src="http://digimonk.net:1630/instagram.png"
                  /></a>
                </li>
                <li style="display: inline-block; padding: 10px 15px">
                  <a href="#"
                    ><img src="http://digimonk.net:1630/youtube.png"
                  /></a>
                </li>
              </ul>
            </td>
  ...
  `;
    ...
    await sendEmail({
      ...
      attachments: [
        {
          filename: req.body.estimationNumber,
          contentType: "application/pdf",
          path: fileUrl,
        },
      ],
    });
    var today = new Date(Number(req.headers["client-time"]));
    var dd = today.getDate();
      ...
      await EstimationModel.findOneAndUpdate(
        { customerLeadId: custId },
        {
          $set: {
            ...
            leadInvoinceNo: req.body.estimationNumber,
          },
        },
        { new: true }
      );
    }
    return res.status(200).json({ msg: "Email Sent", fileUrl });
  } catch (error) {
...
/**
 try {
   ...
   let sentData = new SentEstimationModel({
     // estimateId: req.body.estimateId,
     customerLeadId: req.body.customerLeadId,
     type: "final",
